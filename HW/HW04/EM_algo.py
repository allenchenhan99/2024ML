...
def unravel_index(flat_index, shape):
    row = flat_index // shape[1]
    col = flat_index % shape[1]
    return row, col
        ...
        for class_i in range(10):
            ...
            for pixel in range(len(x_train[0])):
                if x_train[data, pixel]:
                    ...
                    result[class_i] *= (1 - probability[class_i, pixel])
        unknown_class = np.argmax(result)
        count[unknown_class, y_train[data]] += 1

    matching = np.full(10, -1, dtype=np.int32)
    for _ in range(10):
        idx = unravel_index(np.argmax(count), count.shape)
        matching[idx[0]] = idx[1]
        count[idx[0], :] = -1  # Invalidate this row
        count[:, idx[1]] = -1  # Invalidate this column
    return matching
