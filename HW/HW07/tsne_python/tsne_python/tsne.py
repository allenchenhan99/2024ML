...
# def tsne(X=np.array([]), no_dims=2, initial_dims=50, perplexsity=30.0)
def sne(X, labels, mode, no_dims=2, initial_dims=50, perplexity=30.0):
    ...
    for iter in range(max_iter):
        ...
        if (iter + 1) % 10 == 0:
            ...
            img.append(curr_state(Y, labels, mode, perplexity))

        # Stop lying about P-values
        if iter == 100:
            P = P / 4.
    
    # # Debugging: print the length of img list
    # print(f'Number of images captured: {len(img)}')

    # # Save each frame as a separate image for debugging
    # for i, frame in enumerate(img):
    #     frame.save(f'./output/{"t-SNE" if mode == "tSNE" else "symmetric-SNE"}_{perplexity}.png')
    ...
    similarity(P, Q, labels, output_dir)
